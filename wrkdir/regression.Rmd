...
From the above tests I've concluded that Accept, Enroll, F.Undergrad, and Private are associated with Apps
...
model = train(Apps ~ Accept + Enroll + F.Undergrad + Private + Elite, data = clg_norm, method = "lm", trControl = train.control)
```
```{r}
model
summary(model)
...
```{r}
#train.control = trainControl(method = "cv", number = 10)
#model = train(Apps ~ Accept + Enroll + F.Undergrad + Private + Elite, data = clg_norm, method = "leapBackward", trControl = train.control, tuneGrid)
```
